...
async def predict_manual(device_id: Optional[str] = Form("MANUAL_001"), city: Optional[str] = Form(None, description="City name for context/validation."),N: float = Form(...), P: float = Form(...), K: float = Form(...), temperature:float= Form(...),
    ...
    soil_prediction, soil_probs=service.predict_soil_type(image_content)
    
    vc_crop, crop_probs=service.crop_pred( N, P, K, temperature, Humidity, ph, rainfall)
    
    final_crop, compatibility, alternatives, mssg= service.perform_fusion(crop_probs, soil_probs, soil_prediction)
    ...
    log_entry = InferenceLog(
        ...
        raw_tabular_prediction=vc_crop,
...
async def predict_auto( device_id: Optional[str] = Form("MANUAL_001"), N: float = Form(...), P: float = Form(...), K: float = Form(...), 
    ...
    vc_crop, crop_probs=service.crop_pred( N, P, K, temp, humidity, ph, rain)
